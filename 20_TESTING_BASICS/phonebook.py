...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> VALUES >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
MAX_BOOK_SIZE = 100
DEFAULT_FILE_NAME = 'default_book.json'
GOOD_KEYS = ['name', 'lastname', 'tel', 'city', 'state']
MAIN_MENU = [
    '0: QUIT',
    '1: OPEN phonebook',
    '2: CREATE new phone book'
]
BOOK_MENU = ['3: SHOW phonebook',
...
def create_entry(info_list = None, **kwargs):
...
def list_instance(instance, file = 'YOUR BOOK', **kwargs):
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if phone_book is not None:
            show_menu(BOOK_MENU)
        user_choice = input(TEXT_YELLOW + 'Pls enter your choice: '+ RESET_COLORS)
        try:
        ...
        match user_choice:
            ...
            # OPEN phone_book
            case 1:
                file_name = input('Pls enter phonebook name: ')
                phone_book = open_book(file_name, phone_book)
